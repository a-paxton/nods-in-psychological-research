...
arrest_df %>%
  summarize(murder_min = min(Murder),
            murder_mean = mean(Murder),
            murder_max = max(Murder),
            murder_sd = sd(Murder))
...
Getting back to the task at hand: I personally find that base R's `summary()` is
more efficient for very quick introductions to my data. For the
`tidyverse::summarize()` function, we need to specify each variable and the
specific summarization function we want to run. This provides flexibility (e.g.,
our addition of the standard deviation), but you may not always need it. I chose
this example to show the utility of combining the Tidyverse with other
functions: As with any toolbox, you might look elsewhere for the perfect tool
for a job.
